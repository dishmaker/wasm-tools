...
use wasmparser::{
    BinaryReader, DataKind, ElementKind, OperatorsReader, Parser, Payload,
    VisitOperator,
};
...
fn define_benchmarks(c: &mut BenchmarkRegistry) {
    ...
    let test_inputs = collect_benchmark_inputs();
    ...
    let cloned_inputs = {
        ...
        for input in test_inputs.iter() {
            if read_all_wasm(&input.wasm).is_ok() {
                list.push(input.wasm.clone());
            }
        }
        list
    };

    c.register("parse/tests", move |n| {
        
        let start = Instant::now();
        for _ in 0..n {
            for wasm in cloned_inputs.iter() {
    ...
    c.register("validate/tests", move |b| {
    ...
    for file in std::fs::read_dir("benches").unwrap() {
        ...
        c.register(&format!("validate/{name}"), move |b| {
            let start = Instant::now();
            for _ in 0..b {
                validator().validate_all(&wasm).unwrap();
            }
        ...
        //     c.register(&format!("validate-old/{name}"), |b| 
        //         let start = Instant::now();
        //         for _ in 0..b {
        ...
        //     });
        // }
        c.register(&format!("parse/{name}"), move |b| {
            let start = Instant::now();
            for _ in 0..b {
                read_all_wasm(&wasm2).unwrap();
            }
            start.elapsed()
        });
    }
...
fn main() {
    ...
    define_benchmarks(&mut registry);
